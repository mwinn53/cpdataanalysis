---
title: "Analysis of Cyber Patriot 12 Round 1"
author: "mw"
...
```{r setup, echo = FALSE, message = FALSE, warning = FALSE}
  ...
  files <- list.files(path=".", pattern="^CP-XII Round 1.*csv")
  ...
  names(data) <- gsub(x = names(data), pattern = "_cpxii_r1_hsms.", replacement = ".")
  names(data) <- gsub(x = names(data), pattern = "_cpxii_r1_ms.", replacement = ".")
...
The second round will take place November 15-17. The last competition in the regular season will conclude with the State Round on December 6-8.
...
All three teams competed in this round against a total of ```r nrow(data)``` teams in the nation, including ```r nrow(ga)``` in Georgia. 
...
Two of the Evans Middle School teams are in the upper quartile (i.e., top 25%) for the state of Georgia, an indicating the favorable results.
...
>    NOTE: This section is still being developed. The plot should probably be a heatmap.
...
> [TODO] More specifically, all three teams had a significantly higher completion ratio on the Ubuntu 14 images. ```Bits and Bytes``` had a significantly high completion ratio on the Windows 10 image.```Bits and Bytes``` and ```Tax Collectors v2``` had significantly high completion ratios on the Windows 8.1 image. 
