...
```{r input, echo = FALSE, message = FALSE, warning = FALSE}
  ...
  data$Server2008.Completion <- percent(data$Server2008.Found/totals[1])
  data$Ubuntu14.Completion <- percent(data$Ubuntu14.Found/totals[2])
  data$Windows10.Completion <- percent(data$Windows10.Found/totals[3])
...
The graphs below shows the distribution of teams per state, followed by a summary of each state's scores. The numbers along the bottom of the x-axis note the number of teams in each state. The horizontal dashed line represents the national median score (`r median(data$CurrentScore)`) Georgia is highlighted in blue for easy recognition.
...
```{r stateteams}
  ...
  print(
    head(stteams[order(-stteams$Teams),],10)
  )
  
  print(
    ggplot(stteams, aes(x=reorder(State, -Teams), Teams)) +
      geom_bar(stat = "identity") +
      ggtitle("Teams Density by State") + labs(x = "State", y = "Teams")
  )
```
...
```{r score}
  ...
  print(
    ggplot(data, aes(State, CurrentScore)) +
      geom_boxplot() +
...
This graph highlights the Evans Middle School teams' (blue) performance in relation to the teams in Georgia (grey) as well as the teams in Columbia County (red).

```{r gascore}
  print(
    ggplot(data=data[data$State=="GA",], aes(State, CurrentScore)) +
    ...
    xlab("Georgia Teams") +
    ylab("Score") +
    ggtitle("Performance of Georgia Teams")
  )
